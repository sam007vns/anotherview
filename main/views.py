...
def home(request):
	...
	context['show_adsence']= "WebView" in request.user_agent.browser.family
	print(context['show_adsence'])
...
def author_(request,author_name):
	...
	for change in context['author_post']:
		change.url_str=urllib.parse.quote(change.url_str)
	for star in context['recent_reviews']:
		star_rev = star.review_star
		star.review_star = range(star_rev)
	pg_no = 1
...
def redirect_me(request,redirect_link):
	if authors.objects.filter(linkedin_link=redirect_link).exists():
		confirm_redirect = "https://www.linkedin.com/"+redirect_link
...
def add_newsletter(request):
	if request.method == "POST":
		...
		if news_letter.objects.filter(email=email).exists():
			messages.add_message(request,messages.WARNING,"You have alreday susscribed to our newsletter!")
			return redirect('tech-single',article=post_url)
		else:
